...
las = clip_circle(las, 269600, 5168580, 15)
p = sf::st_point(c(269600, 5168580))
p = sf::st_buffer(p, 15)
plot(p, add = T)
dtm = terra::crop(dtm, p)



# ===== DTM ======

# [TIPS] Skip if you already have a DTM and load a DTM instead
...
# Might be useful is some specific case
...
if (display)
{
  ...
  x = plot(filter_poi(las,  hag > 0.5, hag < 1), color = "foliage", pal = foliage.colors) |> add_dtm3d(dtm)
...
if (display)
{
  ...
  x = plot(filter_poi(las,  hag > slice_seeds_at[1], hag < slice_seeds_at[2]), color = "foliage", pal = foliage.colors) |> add_treetops3d(seeds, radius = 0.05, color = col) |> add_dtm3d(dtm)
}
...
las = segment_vegetation(las, seeds, res = 0.05, max_gap = 0.6)
...
{
  x = plot(trees, color = "treeID") |> add_dtm3d(dtm)
  plot(nocircle2, color = "treeID", pal = "red", add = x)
  #x =   plot(trees, color = "foliage", pal = foliage.colors) |> add_dtm3d(dtm)
  lidRtls:::add_circles3d(x, circles$center_x, circles$center_y, circles$radius+0.01, circles$center_z)
}
...
x = plot(tree, color = "foliage", pal = c("chocolate4", "darkgreen"), bg = "white", axis = T)
lidRqsm::plot_qsm(qsm, add = x)
rgl::axes3d()

# Because it looks nice
if (FALSE)
{
  x = lidRqsm::plot_qsm(qsm, bottom_to_zero = FALSE)
