...
#' @export
#' @rdname find_seeds
find_seeds2 <- function(las, params)
{
  foliage <- clusterID <- max_diameter <- passage <- hag <- NULL
  ...
  connectors <- generate_all_groups(circles, step_z = 0.05)
...
# --- Generate 4 points on a disk (along N-S-E-W axes) ---
generate_disk_radii <- function(x, y, z, r) {
  data.frame(
    X = c(x - r, x + r, x, x),
    Y = c(y, y, y - r, y + r),
    Z = rep(z, 4)
