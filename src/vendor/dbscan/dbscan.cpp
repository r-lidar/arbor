...
auto dbscan(const Adaptor& adapt, float eps, int min_pts)
{
  ...
  using  my_kd_tree_t = KDTreeSingleIndexAdaptor<L2_Simple_Adaptor<double, decltype(adapt)>, decltype(adapt), n_cols>;
  using index_t = typename my_kd_tree_t::IndexType;
  using distance_t = typename my_kd_tree_t::DistanceType;
