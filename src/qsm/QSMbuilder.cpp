...
void QSMbuilder::build(const PointCloud& tree)
{
  ...
  {
    std::string msg = "[W1] [QSM Generation Failed] Input data yielded no valid skeleton. A placeholder QSM with 0 volume was generated.";
  ...
  if (!success)
  {
    if (retried)
    {
      std::string msg = "[W2] [No valid measure] No direct measurements matched this tree. QSM generated using allometric reconstruction.";
      ServiceLocator::logger()("\033[33m" + msg + "\033[0m");
      graph.messages.push_back(msg);
    }
    else
    {
      std::string msg = "[W2] [No valid measure] Zero valid measures extracted for this tree. Triggering automatic retry with expanded internode parameters.";
      ServiceLocator::logger()("\033[33m" + msg + "\033[0m");
      retried = true;
