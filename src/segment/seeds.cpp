...
bool is_valid_circle(double radius, double angle_range, double pinlier, double pinside)
{
  ...
  if (radius < 0.10) return (angle_range > 90 && pinlier > 50);
  return (angle_range > 130 && pinlier > 30);
}
...
std::vector<CircleResult> detect_tree_circles(const PointCloud& wood, double resolution = 0.05, int connectivity = 26, int num_ransac_iterations = 400, double inlier_threshold = 0.02, size_t min_cluster_size = 20)
{
  ...
  Grid3D grid(wood.coords[0], wood.coords[1], wood.coords[2], n, resolution);
